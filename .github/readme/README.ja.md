...
ルールは 9 個です。全文は [SKILL.md](../../skills/i-have-adhd/SKILL.md) にあります。

1. 次に取る行動から始める。2分以内に着手できる大きさで。
2. 複数手順のタスクには番号を付ける。
3. 具体的な次の一手を一つ示して終える。
4. 脱線を抑える。
5. 毎ターン状態を言い直し、いま何が動くかを示し、会話より長く残る記録を残す。
6. 所要時間は手順ごとに具体的に（分単位。「少し」も時刻も使わない）。
7. エラーは淡々と。状態を述べ、相手を責めない。
8. リストは切り捨てず順位を付ける。選択肢は3つまで。
9. 前置き、要約、締めの言葉を入れない。
